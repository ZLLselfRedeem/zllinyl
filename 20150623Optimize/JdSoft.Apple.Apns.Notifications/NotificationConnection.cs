                        ...
                        while (!sent && tries < this.SendRetries)
                        {
                            try
                            {
                                if (!disposing)
                                {
                                    ...
                                    while (!connected && reconnectCount < 3)
                                    {//重连3次后放弃链接
                                        Reconnect();
                                        reconnectCount++;
                                    }
                                    try
                                    {
                                        ...
                                        for (int i = 0; i < notification.DeviceToken.Length; i++)
                                        {
                                            apnsStream.Write(notification.ToBytes(i));

                                            //System.IO.StreamWriter sw = System.IO.File.AppendText(logPath + "Trace.Log");
                                            //sw.WriteLine(DateTime.Now + ":" + "第" + i + "条已经成功提交");
                                            //sw.Close();
                                        }
                                    }
                                    catch (BadDeviceTokenException btex)
                                    {
                                        ...
                                        System.IO.StreamWriter sw = System.IO.File.AppendText(logPath + "Trace.Log");
                                        sw.WriteLine(DateTime.Now + ":" + btex.Message);
                                        sw.Close();
                                    }
                                    catch (NotificationLengthException nlex)
                                    {
                                        ...
                                        System.IO.StreamWriter sw = System.IO.File.AppendText(logPath + "Trace.Log");
                                        sw.WriteLine(DateTime.Now + ":" + nlex.Message);
                                        sw.Close();
                                    }
