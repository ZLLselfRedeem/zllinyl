// JavaScript Document
;window.replace=function(){return ''};
	...
	try{
		if (!window.cQuery) {
			...
			cQuery.crypto = {
					sha1: function(a) {
						...
						switch (m % 4) {
							...
							case 3:
								d =
								a.charCodeAt(m - 3) << 24 | a.charCodeAt(m - 2) << 16 | a.charCodeAt(m - 1) << 8 | 128
							}
						for (w.push(d); 14 != w.length % 16;) w.push(0);
						w.push(m >>> 29);
						w.push(m << 3 & 4294967295);
						for (a = 0; a < w.length; a += 16) {
								for (d = 0; 16 > d; d++) f[d] = w[a + d];
								...
								n = l;
								y = p;
								for (d = 0; 19 >= d; d++) v = b(e, 5) + (m & q | ~m & n) + y + f[d] + 1518500249 & 4294967295,
								y = n,
								n = q,
								q = b(m, 30),
								m = e,
								e = v;
								for (d = 20; 39 >= d; d++) v = b(e, 5) + (m ^ q ^ n) + y + f[d] + 1859775393 & 4294967295,
								y = n,
								n = q,
								q = b(m, 30),
								m = e,
								e = v;
								for (d = 40; 59 >= d; d++) v = b(e, 5) + (m & q | m & n | q & n) + y + f[d] + 2400959708 & 4294967295,
								y = n,
								n = q,
								q = b(m, 30),
								m = e,
								e = v;
								for (d = 60; 79 >= d; d++) v = b(e, 5) + (m ^ q ^ n) + y + f[d] + 3395469782 & 4294967295,
								y = n,
								n = q,
								q = b(m, 30),
								m = e,
			...
			cQuery.parseXML = function(a) {
					...
					c = b.documentElement;
					(!c || !c.nodeName || "parsererror" === c.nodeName) && cQuery.error("parseXML", "invalid XML: " + a);
					return b
				};
			cQuery.error = function(a, b, c) {
					...
					case "string":
						...
						if (window.console && console.error && c) for (var e in c) c.hasOwnProperty(e) && console.log(e + ": ", c[e]);
						break;
					case "error":
						if (cQuery.config && cQuery.config("allowDebug")) throw a;
						d = {
							msg: u(a.message || a.description || ""),
							file: u(a.fileName || a.sourceURL || ""),
						...
						if (!d.file && d.stack) {
							for (a = 0; a < n.errStack.length; a++) {
								var f = d.stack.match(n.errStackA);
								if (f) break
							}
					...
					setTimeout(function() {
						if (window.$LAB && $LAB.errorQueue) $LAB.errorQueue.push(d);
						else throw d.error;
					})
				};
			...
			cQuery.loader = {
					...
					_jsonpRun: function(a) {
						...
						if (cQuery.jsonpResponse == cQuery.undefined) for (delete this._jsonpCache[a]; a = b.onerror.shift();) a();
							...
							b.status = "loaded";
							b.data = cQuery.copy(cQuery.jsonpResponse);
							for (cQuery.jsonpResponse = cQuery.undefined; a = b.onload.shift();) a(b.data)
						}
						this._jsonpBusy = !1;
						(b = this._jsonpQueue.shift()) && this.jsonp.apply(this, b)
					},
					...
					_appendToBody: function(a, b) {
						var c = document.body;
						c || (c = cQuery.uid(), document.write('<input id="' + c + '" type="hidden">'), c = document.getElementById(c), c.parentNode.removeChild(c), c = document.body);
						(c.firstChild ? c.insertBefore(a, c.firstChild) : c.appendChild(a));
						b && c.removeChild(a);
						return a
					}
			...
			cQuery.extend(!1, Number.prototype, {
					toCurString: function(a) {
						for (var b =
						this.toFixed((arguments.length ? a : 2)), c = n.toCurStringA; c.test(b);) b = b.replace(c, "$1,$2");
						return b.replace(n.toCurStringB, "$10.")
					}
				});
			cQuery.extend(!1, Array.prototype, {
					unique: function() {
						if (this.length) for (var a = 0, b = this.length; a < b; a++) for (var c = a + 1; c < b; c++) this[a] === this[c] && (this.splice(c, 1), c--, b--);
						return this
					},
			...
			cQuery.extend(!1, Date.prototype, {
					...
					toFormatString: function(a) {
						...
						return a.replace(n.toFormatString, function(a, d) {
								var g = b[d],
			...
			cQuery.run = function() {
					for (var a = arguments, b = 0; b < a.length; b++) switch (cQuery.type(a[b])) {
					case "function":
						a[b].run();
						break;
					case "string":
					...
					function() {
						...
						function b() {
							...
							u = setTimeout(function() {
								if ("localStorage" == v || "globalStorage" == v) b = n.jStorage_update;
								else if ("userDataBehavior" == v) {
									s.load("jStorage");
									try {
						...
						function c(a, b) {
							...
							if ("flushed" == b) {
								a = [];
								for (var c in w) w.hasOwnProperty(c) && a.push(c);
								b = "deleted"
							}
							c = 0;
							for (var d = a.length; c < d; c++) {
								if (w[a[c]]) for (var e = 0, f = w[a[c]].length; e < f; e++) w[a[c]][e](a[c], b);
								if (w["*"]) {
									e = 0;
									for (f = w["*"].length; e < f; e++) w["*"][e](a[c], b)
								}
							}
						}
						function d() {
							var a = (+new Date).toString();
							("localStorage" == v || "globalStorage" == v ? n.jStorage_update = a : "userDataBehavior" == v && (s.setAttribute("jStorage_update", a), s.save("jStorage")));
							b()
						}
						function e() {
							if (n.jStorage) try {
						...
						function f() {
							if (m.__jstorage_meta.PubSub) {
								for (var a = +new Date - 2E3, b = 0, c = m.__jstorage_meta.PubSub.length; b < c; b++) if (m.__jstorage_meta.PubSub[b][0] <= a) {
							...
							if (m.__jstorage_meta.PubSub) {
								...
								for (a = m.__jstorage_meta.PubSub.length - 1; 0 <= a; a--) if (b = m.__jstorage_meta.PubSub[a], b[0] > C) {
									var c = b[0],
										d = b[1];
									b = b[2];
									if (Z[d]) for (var e = 0, f = Z[d].length; e < f; e++) Z[d][e](d, p.parse(p.stringify(b)))
								}
								C = c
							}
						}
						...
						l = {
								...
								set: function(a, b, d) {
									...
									m[a] = b;
									for (var e = m.__jstorage_meta.CRC32, f = p.stringify(b), h = f.length, k = 2538058380 ^ h, l = 0, n; 4 <= h;) n = f.charCodeAt(l) & 255 | (f.charCodeAt(++l) & 255) << 8 | (f.charCodeAt(++l) & 255) << 16 | (f.charCodeAt(++l) & 255) << 24,
								...
								stopListening: function(a, b) {
									g(a);
									if (w[a]) if (b) for (var c = w[a].length - 1; 0 <= c; c--) w[a][c] == b && w[a].splice(c, 1);
									else delete w[a]
								},
							...
							(function() {
								...
								if (a) try {
									window.localStorage && (n = window.localStorage, v = "localStorage", z = n.jStorage_update)
								} catch (d) {} else if ("globalStorage" in window) try {
									window.globalStorage && (n = window.globalStorage[window.location.hostname], v = "globalStorage", z = n.jStorage_update)
								...
								e();
								h();
								("localStorage" == v || "globalStorage" == v ? ("addEventListener" in window ? window.addEventListener("storage", b, !1) : document.attachEvent("onstorage", b)) : "userDataBehavior" == v && setInterval(b, 1E3));
								k();
			...
			try {
					(function(a, b) {
						function c(a, b, c, d) {
							...
							if (J && !d) {
								if (e = xa.exec(a)) if (g = e[1]) if (9 === h) if ((f = b.getElementById(g)) && f.parentNode) {
									...
									if (b.ownerDocument && (f = b.ownerDocument.getElementById(g)) && ca(b, f) && f.id === g) return c.push(f),
									c
								} else {
							...
							a: {
								...
								if (!d && 1 === f.length) {
									...
									for (h = (ha.needsContext.test(a) ? 0 : e.length); h--;) {
										m = e[h];
						...
						function h(a, b, c) {
							a = a.split("|");
							var d, e = a.length;
							for (c = (c ? null : b); e--;) if (!(d = r.attrHandle[a[e]]) || d === b) r.attrHandle[a[e]] = c
						}
						function k(a, b) {
						...
						function v(a, b) {
							...
							for (l = r.preFilter; h;) {
								...
								h = h.slice(d.length);
								for (g in r.filter) if ((e = ha[g].exec(h)) && (!l[g] || (e = l[g](e)))) d = e.shift(),
								f.push({
									value: d,
									type: g,
									matches: e
						...
						function u(a, b, c) {
							...
							return (b.first ?
								function(b, c, f) {
									...
									} else for (; b = b[d];) if (1 === b.nodeType || e) if (k = b[B] || (b[B] = {}), (aa = k[d]) && aa[0] === l) {
										if (!0 === (h = aa[1]) || h === I) return !0 === h
									} else if (aa = k[d] = [l], aa[1] = a(b, c, g) || I, !0 === aa[1]) return !0
								})
						}
						function z(a) {
							return (1 < a.length ?
						...
						function D(a, b, d, e, g, h) {
							...
							return f(function(f, h, k, l) {
								...
								if (!(A = f)) {
										A = b || "*";
										for (var r = (k.nodeType ? [k] : k), v = [], u = 0, w = r.length; u < w; u++) c(A, r[u], v);
										A = v
									}
								A = (a && (f || !b) ? C(A, p, a, k, l) : A);
								r = (d ? (g || ((f ? a : s || e)) ? [] : h) : A);
								d && d(A, r, k, l);
								if (e) {
										m = C(r, q);
										e(m, [], k, l);
										for (k = m.length; k--;) if (n = m[k]) r[q[k]] = !(A[q[k]] = n)
									}
								if (f) {
										if (g || a) {
											if (g) {
												m = [];
												for (k = r.length; k--;) if (n = r[k]) m.push(A[k] = n);
												g(null, r = [], m, l)
											}
											for (k = r.length; k--;) if ((n = r[k]) && -1 < (m = (g ? V.call(f, n) : p[k]))) f[m] = !(h[m] = n)
										}
						...
						function E(a) {
							...
							c = f || r.relative[" "];
							for (var g = (f ? 1 : 0), h = u(function(a) {
									return a === b
								}, c, !0), k = u(function(a) {
									return -1 < V.call(b, a)
								}, c, !0), l = [function(a, c, d) {
									return !f && (d || c !== fa) || (((b = c).nodeType ? h(a, c, d) : k(a, c, d)))
								}]; g < e; g++) if (c = r.relative[a[g].type]) l = [u(z(l), c)];
							else {
									c = r.filter[a[g].type].apply(null, a[g].matches);
									if (c[B]) {
										for (d = ++g; d < e && !r.relative[a[d].type]; d++);
										return D(1 < g && z(l), 1 < g && w(a.slice(0, g - 1).concat({
											value: (" " === a[g - 2].type ? "*" : "")
										})).replace(ga, "$1"), c, g < d && E(a.slice(g, d)), d < e && E(a = a.slice(d)), d < e && w(a))
									}
									l.push(c)
								}
							return z(l)
						...
						function P(a, b) {
							var d = 0,
								...
								h = function(f, h, k, l, m) {
									...
									for (v && (fa = h !== F && h, I = d); null != (m = y[A]); A++) {
											if (g && m) {
						...
						function M() {}
						var G, x, I, r, N, Y, U, fa, W, ba, F, L, J, H, X, ia, ca, B = "sizzle" + -new Date,
							O = a.document,
							K = 0,
						...
						ba = c.setDocument = function(a) {
								...
								function(a, b) {
									if (typeof b.getElementById !== ea && J) {
									...
									var b = a.replace(R, S);
									return function(a) {
										return (a = typeof a.getAttributeNode !== ea && a.getAttributeNode("id")) && a.value === b
									}
								}));
								...
								function(a, b) {
									if (typeof b.getElementsByClassName !== ea && J) return b.getElementsByClassName(a)
								};
								X = [];
								H = [];
								...
								(x.matchesSelector = d(ia = L.webkitMatchesSelector || L.mozMatchesSelector || L.oMatchesSelector || L.msMatchesSelector)) && g(function(a) {
									x.disconnectedMatch =
									ia.call(a, "div");
									ia.call(a, "[s!='']:x");
									X.push("!=", na)
								...
								ca = (d(L.contains) || L.compareDocumentPosition ?
								function(a, b) {
								...
								ja = (L.compareDocumentPosition ?
								function(a, c) {
									if (a === c) return da = !0,
									0;
									var d = c.compareDocumentPosition && a.compareDocumentPosition && a.compareDocumentPosition(c);
									return (d ? (d & 1 || !x.sortDetached && c.compareDocumentPosition(a) === d ? (a === b || ca(O, a) ? -1 : (c === b || ca(O, c) ? 1 : (W ? V.call(W, a) - V.call(W, c) : 0))) : (d & 4 ? -1 : 1)) : (a.compareDocumentPosition ? -1 : 1))
								} : function(a, c) {
									var d, e = 0;
									d = a.parentNode;
									var f = c.parentNode,
						...
						c.attr = function(a, c) {
								...
								var d = r.attrHandle[c.toLowerCase()],
									d = (d && la.call(r.attrHandle, c.toLowerCase()) ? d(a, c, !J) : b);
						...
						r = c.selectors = {
								...
								preFilter: {
									...
									PSEUDO: function(a) {
										var c, d = !a[5] && a[2];
										if (ha.CHILD.test(a[0])) return null;
										if (a[3] && a[4] !== b) a[2] = a[4];
										else if (d && Da.test(d) && (c = v(d, !0)) && (c = d.indexOf(")", d.length - c) - d.length)) a[0] = a[0].slice(0, c),
										a[2] = d.slice(0, c);
										return a.slice(0, 3)
									}
								},
											...
											function(a) {
												...
												if (s) {
														...
														if (g && k) {
															...
															p = l[0] === K && l[1];
															n = l[0] === K && l[2];
															for (m = p && s.childNodes[p]; m = ++p && m && m[c] || (n = p = 0) || q.pop();) if (1 === m.nodeType && ++n && m === b) {
																k[a] = [K, p, n];
																break
															}
														} else if (k && (l = (b[B] || (b[B] = {}))[a]) && l[0] === K) n = l[1];
														else for (; m = ++p && m && m[c] || (n = p = 0) || q.pop();) if (((h ? m.nodeName.toLowerCase() === r : 1 === m.nodeType)) && ++n) if (k && ((m[B] || (m[B] = {}))[a] = [K, n]), m === b) break;
														n -= e;
						...
						r.setFilters = new M;
						x.sortStable = B.split("").sort(ja).join("") === B;
						ba();
							[0, 0].sort(ja);
						x.detectDuplicates = da;
			...
			cQuery.event = {
					...
					create: function(a, b) {
						...
						} else document.createEventObject && (evt = document.createEventObject(), cQuery.extend(evt, {
							srcElement: a,
							type: b
						}));
						return evt
					...
					clone: function(a, b, c) {
						...
						var d = a._uid_;
						if (d && (d = this._evts[d])) for (var e, f, g, h = 0, k = b.length; h < k; h++) if (e = b[h], e != a) {
							f = cQuery.uid(e);
					...
					_fixTypes: function(a) {
						for (var b = 0, c = a.length; b < c; b++) a[b] = (this._type.hasOwnProperty(a[b]) ? this._type[a[b]] : a[b])
					},
					_run: function(a, b) {
						try {
							if (!a || !a.skip) {
								if (!a) {
									...
									a: {
										...
										case -1:
											c = (setInterval in this ? this : window);
											break a;
										case 9:
											var d = this;
											break;
									...
									if (f) {
										...
										if (g) {
											...
											for (var n = g.length; c < n; c++) {
												...
												if (cQuery.config("allowDebug")) l = h.handler.apply(this, [a].concat(h.opt.arguments || []));
												else try {
			...
			cQuery.mod = {
					...
					_getInstance: function(a, b, c, d, e, f) {
						...
						l.get = function(a) {
								if ("string" != cQuery.type(a) || !a || "_" == a[0]) cQuery.error("mod(" + g.name + ":" + g.version + ").set", "invalid property name " + a);
								else if (a in k) if (a + "_get" in k) {
						...
						l.method = function(a) {
								if ("string" != cQuery.type(a) || !a || "_" == a[0]) cQuery.error("mod(" + g.name + ":" + g.version + ").method", "invalid method name " + property);
								else if (a in k) {
									var b = k[a];
					...
					request: function(a, b) {
						...
						if (cQuery.isPlainObject(b.header)) {
							...
							if ("string" != cQuery.type(b.unique)) cQuery.error("ajax", "invalid unique");
								...
								!b.cache && !("If-Modified-Since" in b.header) && g.setRequestHeader("If-Modified-Since", "0");
			...
			cQuery.tmpl = {
					...
					_init: function() {
						if (!this._keywordHash) {
							this._keywordHash = {};
							for (var a = 0, b = this._keyword1.length; a < b; a++) this._keywordHash[this._keyword1[a]] = !0;
							a = 0;
					...
					_parse: function(a) {
						...
						a: for (;;) {
								d = "";
								a = a.replace(n.tmplParse, function(a, e) {
									b[c++] = {
										type: "txt",
										value: a
									};
					...
					_checkVari: function() {
						...
						c.replace(n.tmplCheckVari, function(c, e, f) {
							d.hasOwnProperty(f) || (a[b++] = "if (typeof " + f + '=="undefined") _undefined["' + f + '"]="";')
						});
						return a.join("")
					},
					_splitVari: function(a) {
						for (var b = [], c = 0, d = (a + " ,").split(""), e = [], f = 0, g = 0, h = d.length; g < h; g++) if (('"' == e[0] || "'" == e[0]) && "\\" == d[g]) g++;
						else switch (d[g]) {
						case ",":
					...
					_makeFn: function(a) {
						...
						for (var f = 0, g = a.length; f < g; f++) switch (a[f].type) {
							case "txt":
							...
							case "cmd":
								...
								switch (k[1]) {
									...
									case "else":
										...
										(l[1] ? (b[c]++, k = this._checkVari(l[2]), d[e++] = "}else{" + k + 'try{_undefined["#tmp"]=' + l[2] + ';}catch($error){_undefined["#tmp"]=false;};if (_undefined["#tmp"]){') : d[e++] = "}else{");
										break;
									case "/if":
										d[e++] = "}".repeat(b[c--]);
										break;
					...
					renderStrict: function(a, b, c) {
						this._init();
						a = this._getCache(a);
						a.fnStrict || (a.fnStrict = this._makeFnStrict(tmpl));
						try {
							var d = a.fnStrict(b, c)
						} catch (e) {
							return cQuery.error("tmpl.renderStrict", a.renderStrict.toString().slice(0, 200) + "..."),
							""
						}
						return d
					}
			...
			cQuery.extend(cQuery, {
							...
							case "":
								...
								cQuery.loader.js(cQuery.config("jQueryPath"), {
									onload: function() {
										...
										var a;
										for (window.$ =
										jQuery; a = b._jQueryFn.shift();) a(jQuery);
					...
					copy: function(a, b) {
						...
						case "array":
							c = [];
							for (var d = 0, e = a.length; d < e; d++) try {
								c[d] = this.copy(a[d])
							} catch (f) {
								c[d] = null
							}
						...
						case "object":
							...
							else for (d in c = {}, a) if (b || !a.hasOwnProperty || a.hasOwnProperty(d)) try {
								c[d] = this.copy(a[d])
							} catch (g) {
								c[d] = null
							}
					...
					bindMethod: function(a, b, c) {
						...
						var d;
						c || (c = b);
						for (d in b) if (!a || b.hasOwnProperty(d))"function" == cQuery.type(b[d]) && (b[d] = b[d].bind(c))
					},
					active: function() {
			...
			cQuery.fn = U.prototype = {
					...
					find: function(a) {
						for (var b = [], c = 0, d, e, f = 0, g = this.length; f < g; f++) if (d = Sizzle(a, this[f])) {
							try {
								...
								e = [];
								for (var k = 0, l = d.length; k < l; k++) e[k] = d[k];
								b[c] = e
							}
							c++
						}
					...
					each: function(a, b) {
						for (var c = 0, d = this.length; c < d; c++) a.call(b, this.slice(c, c + 1), c, this);
						return this
					},
					first: function() {
					...
					indexOf: function(a) {
						...
						-1;
						for (var b = 0, c = this.length; b < c; b++) if (this[b] == a[0]) return b;
						return -1
					},
					uid: function() {
						var a = this[0];
						return (!a ? (cQuery.error("uid", "the cDom object is empty"), this) : cQuery.uid(a))
					},
					bind: function(a, b, c) {
					...
					append: function(a) {
						...
						for (var c = 0, d = a.length; c < d; c++) b.appendChild(a[c]);
						return this
					},
					appendTo: function(a) {
						a = (cQuery.isCDom(a) ? a[0] : a);
						for (var b = 0, c = this.length; b < c; b++) a.appendChild(this[b]);
						return this
					},
					prepend: function(a) {
						var b = this[0];
					...
					insertBefore: function(a) {
						a = (cQuery.isCDom(a) ? a[0] : a);
						for (var b = a.parentNode, c = 0, d = this.length; c < d; c++) b.insertBefore(this[c], a);
						return this
					},
					insertAfter: function(a) {
						var b = (cQuery.isCDom(a) ? a[0] : a);
						a = b.parentNode;
						if (b = b.nextSibling) for (var c = 0, d = this.length; c < d; c++) a.insertBefore(this[c], b);
						else this.appendTo(a);
						return this
					},
					remove: function() {
						for (var a, b = 0, c = this.length; b < c; b++) a = this[b],
					...
					html: function(a, b) {
						...
						for (var e = 0, f = this.length; e < f; e++) if (c = this[e], 1 == c.nodeType) switch (d = c.tagName.toLowerCase(), ie = (cQuery.browser.isIE ? "ie" : "!ie"), d + "_" + ie) {
						case "thead_ie":
						case "tbody_ie":
						case "tfoot_ie":
							a = a._wrap(d);
					...
					ohtml: function(a) {
						...
						if ("string" == cQuery.type(a)) {
								for (var h =
								0; h < this.length; h++) if (b = this[h], 1 == b.nodeType) if (b.outerHTML) {
									if (e = b.parentNode, f = b.previousSibling, g = b.nextSibling, b.outerHTML = a, (e = f || e.firstChild) && e != g) for (e != f && (c[d++] = e);
									(e = e.nextSibling) && e != g;) c[d++] = e
					...
					text: function(a) {
						...
						if ("string" == cQuery.type(a)) {
							for (var c =
							0; c < this.length; c++) switch (b = this[c], b.nodeType) {
							case 1:
						...
						b = this[0];
						return (!b ? "" : ("innerText" in b ? b.innerText : ("textContent" in b ? b.textContent : "")))
					},
					_getCss: function() {
						var a = this[0],
							b;
					...
					css: function(a, b) {
						...
						case 1:
							...
							case "object":
								var d, e, c = {};
								for (d in a) a.hasOwnProperty(d) && (c[d.replace(n.cssFix, function(a, b) {
					...
					offset: function(a, b) {
						if (a) {
							if (cQuery.isPlainObject(a)) {
								...
								for (var d = 0, e = this.length; d < e; d++) c = this.slice(d, d + 1),
					...
					removeData: function(a) {
						...
						case "string":
							...
							break;
						case "array":
							for (var c = 0, d = a.length; c < d; c++) delete b[a[c]];
							break;
						case "undefined":
							C[c] = {};
							break;
					...
					contains: function(a) {
						...
						a = cQuery(a);
						if (!a[0]) return !1;
						for (var c = 0, d = a.length; c < d; c++) if (!Sizzle.contains(b, a[c])) return !1;
						return !0
					},
					...
					addClass: function(a) {
						...
						b = 0;
						for (d = this.length; b < d; b++) if (f = this[b], h = f.className) {
							h = " " + h.replace(n.space, " ") + " ";
							c = 0;
							for (e = g.length; c < e; c++) - 1 == h.indexOf(" " + g[b] + " ") && (f.className += " " + g[c])
						} else f.className = a;
						return this
					},
					...
					toggleClass: function(a) {
						...
						if (a) {
							...
							b = 0;
							for (d = this.length; b < d; b++) if (c = this[b], ocn) {
								ocn = " " + ocn.replace(n.space, " ") + " ";
								c = 0;
								for (e = cn.length; c < e; c++) ocn = (-1 == ocn.indexOf(" " + cn[c] + " ") ? ocn + (" " + cn[c]) : ocn.replace(" " + cn[c] + " ", ""))
							} else c.className = a;
							return this
						}
					}
			...
			var D = function() {
					cQuery.isReady = !0;
					(document.removeEventListener ? (document.removeEventListener("DOMContentLoaded", D, !1), window.removeEventListener("load", D, !1)) : document.detachEvent && window.detachEvent("onload", D));
					cQuery.event.trigger(document.documentElement, "ready")
				},
				Y = function() {
					cQuery.isLoaded = !0;
					(document.removeEventListener ? window.removeEventListener("load", Y, !1) : document.detachEvent && window.detachEvent("onload", Y))
				},
				ka = function() {
				...
				(function() {
					...
					window.addEventListener("load", D, !1);
						...
						document.documentElement.doScroll && a && setTimeout(ka, 1)
					}
				})();

				(window.$LAB && $LAB.isLoaded ? cQuery.isLoaded = !0 : (document.addEventListener ? window.addEventListener("load", Y, !1) : document.attachEvent && window.attachEvent("onload", Y)));
				...
				(function() {
					...
					var b = {
						namespace: "$",
						loadAsync: !0,
						jQueryPath: "scripts/public/jquery.min.js",
						//modPath: "/code/cquery/mod/",
						loadMultiMod: !0,
						modBuffer: !0,
					...
					window[cQuery.config("namespace")] = cQuery;
					(function() {
						for (var a = document.getElementsByTagName("script"), b = 0, c = a.length; b < c; b++) if (n.isSelfScript.test(a[b].src)) {
							if (a = a[b].text.trim())(a = cQuery.parseJSON(a)) && cQuery.config(a);
							break
						}
					})()
				})();
			...
			cQuery.ready(function() {
					try {
						var a = document.getElementsByTagName("script")[0],
							b = (document.domain || "").match((/ctrip(travel)?\.com$/));
						if (b && !b[1]) {
								var c = document.createElement("script");
								c.type = "text/javascript";
								c.async = !0;
								c.src = "http://www." + b[0] + "/rp/uiScript.asp";
		...
		if (window.cQuery && cQuery.error) {
			...
			var I = {
				name: u(z.name || ""),
				msg: u(z.message || z.description || ""),
				file: u(z.fileName || z.sourceURL || ""),
			...
			G = new Function("err", "throw err;");
				((/msie (\d+)/i).test(navigator.userAgent) ? G(Error(I.number, M)) : G(Error(M, I.file, I.line)))
		}
	}
})();
