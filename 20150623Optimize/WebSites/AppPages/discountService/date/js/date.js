...
(function ($) {      
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        $.fn.date.defaultOptions = {
            ...
            mode:null,                       //操作模式（滑动模式）
            event:"focus",                    //打开日期插件默认方式为点击后后弹出日期 
            show:true
        }
        //用户选项覆盖插件默认选项   
	...
	function resetIndex(){
        ...
        function extendOptions(){
            $("#datePage").show(); 
            $("#dateshadow").show();
            $("#dateshadow").css({"height": $(document).height() + "px"});
            noScroll();
        }
        //日期滑动
            ...
            var strM = $("#monthwrapper ul li:eq("+indexM+")").html().substr(0,$("#monthwrapper ul li:eq("+indexM+")").html().length-1);
              yearScroll = new iScroll("yearwrapper",{snap:"li",vScrollbar:false,
                  onScrollEnd:function () {
                       ...
                       var y = $("#yearwrapper ul li:eq("+indexY+")").html().substr(0,$("#yearwrapper ul li:eq("+indexY+")").html().length-1);
                       var m = $("#monthwrapper ul li:eq("+indexM+")").html().substr(0,$("#monthwrapper ul li:eq("+indexM+")").html().length-1);
                       //opts.endday = checkdays(strY,strM);
                       opts.endday = chkDay(y, m);
        ...
        //检查当前年月时日期的天数
        function chkDay(y, m){
          if( m == '01' || m == '03' || m == '05' || m == '07' || m == '08' || m == '10' || m == '12' ) { opts.endday = 31; }
          else if( m == '04' || m == '06' || m == '09' || m == '11' ) { opts.endday = 30; }
          else if( m == '02'){
