	...
	Calendar.prototype = {
		...
		*/
		getClass: function (date, tempDateStr, input) {
			...
			//var nowDate = new Date('2013/10/01');//设置“可设定最小日期”的最小日期；
			var tempDay = (date - nowDate) / (24 * 60 * 60 * 1000);
			*/
			var beenDate = new Date('2000/10/01');
			var tempDay = (date - beenDate) / (24 * 60 * 60 * 1000);
			...
			if (this.input && !$(tag).hasClass(obj.classNames.nothismonth) && tag.id) {
				...
				if ($(this.input).data('showWeek') * 1) {
					...
					if (tips && this.input.value.isDate()) {
						...
						//if (txt && this.input.clientWidth >= 105) {//星期显示
						if (txt && this.input.clientWidth >= 105) {
							var charset = $.config("charset");
							function weekpng(){
								$(tips).css({
									"background-image": "url(http://pic.c-ctrip.com/cquery/" + txt + ".png)",//日期文字图片
									"background-position": "right center",
									"background-repeat": "no-repeat"
